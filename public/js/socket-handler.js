...
function setConnectionStatus(status) {
  ...
  if (!el) { el = document.createElement('div'); el.id='connection-status'; el.className='fixed top-16 left-1/2 -translate-x-1/2 z-[60] px-3 py-1.5 rounded-full text-xs font-bold shadow-lg border backdrop-blur'; document.body.appendChild(el); }
  const states={connected:['Онлайн','hidden'],restored:['Соединение восстановлено','success'],reconnecting:['Соединение потеряно · восстанавливаем игру…','warning'],expired:['Сессия завершена','error']};
  const current=states[status]||states.connected;el.innerText=current[0];el.classList.remove('hidden','bg-emerald-950','text-emerald-300','border-emerald-700','bg-amber-950','text-amber-300','border-amber-700','bg-red-950','text-red-300','border-red-700');
  if(current[1]==='hidden')el.classList.add('hidden');else if(current[1]==='success')el.classList.add('bg-emerald-950','text-emerald-300','border-emerald-700');else if(current[1]==='warning')el.classList.add('bg-amber-950','text-amber-300','border-amber-700');else el.classList.add('bg-red-950','text-red-300','border-red-700');
}
const SocketHandler = {
  ...
  initListeners(){
    if(window.UI)UI.setConnectionStatus=setConnectionStatus;
    socket.on('connect',()=>setConnectionStatus(initialSessionId||window.BUNKER_PLAYER_ID?'reconnecting':'connected'));
    socket.on('reconnect_attempt',()=>setConnectionStatus('reconnecting')); socket.on('disconnect',()=>setConnectionStatus('reconnecting')); socket.on('connect_error',()=>setConnectionStatus('reconnecting'));
    socket.on('session:issued',(data)=>{setSession(data?.sessionId,data?.playerId);setConnectionStatus('connected');});
    socket.on('session:restored',(data)=>{setSession(socket.auth?.sessionId,data?.playerId);UI.updateGameState(data.state);setConnectionStatus('restored');});
    socket.on('session:expired',(data)=>{clearSession();setConnectionStatus('expired');UI.showPrivateModal?.('Сессия завершена',data?.message||'Игровая сессия больше недоступна.');});
