document.addEventListener('DOMContentLoaded', () => {
  ...
  panel.innerHTML = `
    ...
    <label class="block min-w-0"><span class="block text-xs font-semibold text-gray-200 mb-1">Режим</span><select id="game-mode-select" class="w-full min-w-0 bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white"><option value="classic">Classic</option></select><span class="block text-[11px] text-gray-500 mt-1">Classic — текущая игровая логика Bunker.</span></label>
    <label class="flex items-start justify-between gap-3 bg-gray-900/70 border border-gray-700 rounded-lg p-3 cursor-pointer"><span class="min-w-0"><span class="block text-xs font-semibold text-gray-200">Дополнительные задания</span><span class="block text-[11px] text-gray-500 mt-1">Текущие секретные personal goals.</span></span><input id="additional-tasks-toggle" type="checkbox" class="mt-1 w-5 h-5 accent-amber-500 shrink-0"></label>
    <label class="flex items-start justify-between gap-3 bg-gray-900/70 border border-gray-700 rounded-lg p-3 cursor-pointer"><span class="min-w-0"><span class="block text-xs font-semibold text-gray-200">Характеристики выживания</span><span class="block text-[11px] text-gray-500 mt-1">Вода, еда, энергия, медикаменты и состояния.</span></span><input id="survival-stats-toggle" type="checkbox" class="mt-1 w-5 h-5 accent-amber-500 shrink-0"></label>
    <label class="flex items-start justify-between gap-3 bg-gray-900/70 border border-gray-700 rounded-lg p-3 cursor-pointer"><span class="min-w-0"><span class="block text-xs font-semibold text-gray-200">Секретный Предатель</span><span class="block text-[11px] text-gray-500 mt-1">Существующая настройка Classic.</span></span><input id="traitor-toggle" type="checkbox" class="mt-1 w-5 h-5 accent-amber-500 shrink-0"></label>`;
  ...
  UI.updateGameState = (state) => {
    ...
    panel.querySelectorAll('select, input').forEach((control) => { control.disabled = !isHost || state.status !== 'LOBBY'; });
  ...
  if (UI.currentState) UI.updateGameState(UI.currentState);
});
