...
function createRateLimiter(){const buckets=new Map();return(socketId,key,limit,windowMs)=>{const now=Date.now(),bucketKey=`${socketId}:${key}`,bucket=buckets.get(bucketKey)||{start:now,count:0};if(now-bucket.start>=windowMs){bucket.start=now;bucket.count=0;}bucket.count++;buckets.set(bucketKey,bucket);return bucket.count<=limit;};}const allowEvent=createRateLimiter();
function playerAction(room,socket,fn){if(!room||room.status==="LOBBY"||room.status==="GAME_OVER")return;const playerId=roomManager.getPlayerIdBySocket(socket.id);const player=playerId?room.players.get(playerId):null;if(!player||player.eliminated)return;return fn(room,player,playerId);}
function rBroadcast(room){room.broadcastState();}
io.on("connection",socket=>{
 console.log(`[Socket] Новое подключение: ${socket.id}`);
 const presentedSessionId=socket.handshake.auth?.sessionId;
 if(presentedSessionId){
   const restored=roomManager.reconnect(socket,presentedSessionId);
   if(!restored.success)socket.emit("session:expired",{code:restored.code,message:restored.message});
 }
 ...
 socket.on("card:reveal",payload=>{if(!allowEvent(socket.id,"cards",20,10000))return;const room=roomManager.getRoomBySocket(socket.id),playerId=roomManager.getPlayerIdBySocket(socket.id),category=stringValue(payload?.category,40);if(room&&playerId&&category)room.revealCard(playerId,category);});
 socket.on("card:action",payload=>{if(!allowEvent(socket.id,"cards",20,10000))return;const room=roomManager.getRoomBySocket(socket.id),playerId=roomManager.getPlayerIdBySocket(socket.id),category=stringValue(payload?.category,40),targetId=stringValue(payload?.targetId,100)||null;if(room&&playerId&&category)room.useSpecialCard(playerId,category,targetId);});
 socket.on("vote:cast",payload=>{if(!allowEvent(socket.id,"vote",30,10000))return;const room=roomManager.getRoomBySocket(socket.id),playerId=roomManager.getPlayerIdBySocket(socket.id),targetId=stringValue(payload?.targetId,100);if(!room||!playerId||!targetId)return;if(room.status!=="VOTING")return;if(room.votes.has(playerId)){socket.emit("action:private",{title:"Ошибка",message:"Вы уже проголосовали в этом голосовании."});return;}room.castVote(playerId,targetId);});
 socket.on("alliance:create",payload=>{if(!allowEvent(socket.id,"alliance",10,10000))return;const room=roomManager.getRoomBySocket(socket.id),targetId=stringValue(payload?.targetId,100);const result=playerAction(room,socket,(r,p,id)=>r.proposeAlliance(id,targetId));if(result&&!result.success)socket.emit("error:msg",result.message);else if(result)rBroadcast(room);});
 socket.on("alliance:accept",payload=>{if(!allowEvent(socket.id,"alliance",10,10000))return;const room=roomManager.getRoomBySocket(socket.id),fromId=stringValue(payload?.fromId,100);const result=playerAction(room,socket,(r,p,id)=>r.acceptAlliance(id,fromId));if(result&&!result.success)socket.emit("error:msg",result.message);else if(result)rBroadcast(room);});
 socket.on("alliance:break",payload=>{if(!allowEvent(socket.id,"alliance",10,10000))return;const room=roomManager.getRoomBySocket(socket.id),allianceId=stringValue(payload?.allianceId,200);const result=playerAction(room,socket,(r,p,id)=>r.breakAlliance(id,allianceId));if(result&&!result.success)socket.emit("error:msg",result.message);else if(result)rBroadcast(room);});
 socket.on("trust:update",payload=>{if(!allowEvent(socket.id,"trust",15,10000))return;const room=roomManager.getRoomBySocket(socket.id),targetId=stringValue(payload?.targetId,100),delta=Number(payload?.delta);const result=playerAction(room,socket,(r,p,id)=>r.updateTrust(id,targetId,delta));if(result&&!result.success)socket.emit("error:msg",result.message);else if(result)rBroadcast(room);});
 socket.on("state:heal",payload=>{if(!allowEvent(socket.id,"heal",10,10000))return;const room=roomManager.getRoomBySocket(socket.id),targetId=stringValue(payload?.targetId,100);const result=playerAction(room,socket,(r,p,id)=>r.healPlayer(id,targetId));if(result&&!result.success)socket.emit("error:msg",result.message);else if(result)rBroadcast(room);});
 socket.on("traitor:sabotage",payload=>{if(!allowEvent(socket.id,"sabotage",5,10000))return;const room=roomManager.getRoomBySocket(socket.id),action=stringValue(payload?.action,50);const result=playerAction(room,socket,(r,p,id)=>r.traitorSabotage(id,action));if(result&&!result.success)socket.emit("error:msg",result.message);else if(result)rBroadcast(room);});
 socket.on("voice:signal",payload=>{if(!allowEvent(socket.id,"voice-signal",250,10000))return;const room=roomManager.getRoomBySocket(socket.id),senderId=roomManager.getPlayerIdBySocket(socket.id),targetId=stringValue(payload?.targetId,100);if(!room||!senderId||!targetId||targetId===senderId||!validSignal(payload?.signal))return;const targetPlayer=room.players.get(targetId);if(!targetPlayer||targetPlayer.eliminated)return;io.to(targetId).emit("voice:signal",{senderId,signal:payload.signal});});
 socket.on("voice:speaking",payload=>{if(!allowEvent(socket.id,"voice-speaking",20,10000))return;const room=roomManager.getRoomBySocket(socket.id),playerId=roomManager.getPlayerIdBySocket(socket.id);if(room&&playerId&&typeof payload?.isSpeaking==="boolean")room.broadcastSpeaking(playerId,payload.isSpeaking);});
 socket.on("disconnect",()=>{console.log(`[Socket] Игрок отключился: ${socket.id}`);roomManager.handleDisconnect(socket.id);});
});
const PORT=process.env.PORT||3000,HOST="0.0.0.0",LOCAL_IP=getLocalIp();if(require.main===module)server.listen(PORT,HOST,()=>{console.log("====================================================");console.log("   🔥 ПОСТАПОКАЛИПТИЧЕСКАЯ ВЕБ-ИГРА «БУНКЕР» 🔥     ");console.log("====================================================");console.log(` Сервер запущен на хосте: ${HOST}:${PORT}`);console.log("====================================================");});module.exports=app;
