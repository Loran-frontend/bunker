...
app.get("/api/voice-config", (req, res) => {
  const url = (process.env.TURN_URL || "").trim(), username = process.env.TURN_USERNAME || "", credential = process.env.TURN_CREDENTIAL || "";
  res.json(url && username && credential ? { iceServers: [{ urls: url, username, credential }] } : { iceServers: [] });
});
...
function validSignal(signal) {
  ...
  if (signal.sdp) return typeof signal.sdp === "object" && typeof signal.sdp.type === "string" && ["offer", "answer", "pranswer", "rollback"].includes(signal.sdp.type) && typeof signal.sdp.sdp === "string" && signal.sdp.sdp.length <= 20000;
  if (signal.candidate) return typeof signal.candidate === "object" && typeof signal.candidate.candidate === "string" && signal.candidate.candidate.length <= 5000;
  return false;
}
function createRateLimiter() { const buckets = new Map(); return (socketId, key, limit, windowMs) => { const now = Date.now(), bucketKey = `${socketId}:${key}`, bucket = buckets.get(bucketKey) || { start: now, count: 0 }; if (now - bucket.start >= windowMs) { bucket.start = now; bucket.count = 0; } bucket.count += 1; buckets.set(bucketKey, bucket); return bucket.count <= limit; }; }
const allowEvent = createRateLimiter();
function playerAction(room, socket, fn) { if (!room || room.status === "LOBBY" || room.status === "GAME_OVER") return; const player = room.players.get(socket.id); if (!player || player.eliminated) return; return fn(room, player); }
...
const PORT = process.env.PORT || 3000, HOST = "0.0.0.0", LOCAL_IP = getLocalIp();
if (require.main === module) server.listen(PORT, HOST, () => { console.log("===================================================="); console.log("   🔥 ПОСТАПОКАЛИПТИЧЕСКАЯ ВЕБ-ИГРА «БУНКЕР» 🔥     "); console.log("===================================================="); console.log(` Сервер запущен на хосте: ${HOST}:${PORT}`); console.log(` 🌐 Игра доступна по адресу: http://${LOCAL_IP}:${PORT}`); console.log("===================================================="); });
module.exports = app;
