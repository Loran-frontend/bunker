...
class FinaleMechanics {
  ...
  evaluate(survivors) {
    ...
    const ratios = Object.fromEntries(
      Object.keys(resources).map((key) => [key, resources[key] / Math.max(1, mechanics.maxResources?.[key] || 1)]),
    );
    const resourceScore = Math.round(
      (Object.values(ratios).reduce((sum, value) => sum + value, 0) / Object.keys(ratios).length) * 100,
    );
    const criticalResources = Object.entries(ratios)
      .filter(([, ratio]) => ratio < Config.resources.criticalThreshold)
      .map(([key]) => key);
    ...
    const specialists = specialistRoles.filter((role) =>
      survivors.some((player) => String(player.cards?.professions?.value || '').toLowerCase().includes(role)),
    ).length;
    ...
    let outcome = 'success';
    if (
      criticalResources.length >= 2 ||
      (criticalResources.length === 1 && criticalResources[0] !== 'medicine' && resourceScore < 45)
    ) {
      outcome = 'failure';
    } else if (traitorAlive && criticalResources.length > 0) {
      outcome = 'traitor';
    } else if (
      resourceScore < 40 ||
      severe >= Math.max(2, Math.ceil(survivors.length * 0.5)) ||
      specialists < 2
    ) {
    ...
    const titles = {
      success: 'Успешное выживание',
      consequences: 'Выживание с последствиями',
      failure: 'Провал',
      traitor: 'Победа предателя',
    };
  ...
  generateRuleBasedStory(survivors) {
    ...
    if (checks.outcome === 'traitor') {
      return `💥 Бункер пал под ударом саботажа.\n\nВыжившие: ${names}.\n\nПОБЕДА ПРЕДАТЕЛЯ.`;
    }
    if (checks.outcome === 'failure') {
      return `🔴 Критические ресурсы исчерпаны, и бункер не смог обеспечить безопасное выживание.\n\nВыжившие: ${names}.\n\nБУНКЕР ОБРЕЧЁН.`;
    }
    if (checks.outcome === 'consequences') {
      return `🟡 Бункер пережил кризис, но ценой серьёзных последствий.\n\nВыжившие: ${names}.\n\nВЫЖИВАНИЕ С ПОСЛЕДСТВИЯМИ.`;
    }
  ...
  async generateAiFinaleStory(survivors, hasTraitor) {
    ...
    try {
      ...
      if (typeof responseText !== 'string' || !responseText.trim()) {
        throw new Error('AI API returned empty or malformed response');
      }
...
function installFinaleMechanics(GameState) {
  ...
  GameState.prototype.checkGameOver = function checkGameOver() {
    ...
    this.getFinaleMechanics().resolve(alive).catch((error) => {
      console.error('[Finale Error]', error);
    });
    this.broadcastState();
    return true;
  };
  ...
  GameState.prototype.__legacyCheckGameOver = originalCheckGameOver;
}
