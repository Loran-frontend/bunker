...
module.exports = function installHostGameSettings(GameState) {
  ...
  GameState.prototype.updateSettings = function updateSettings(playerId, patch) {
    if (this.status !== 'LOBBY') {
      return { success: false, message: 'Настройки игры уже заблокированы после старта.' };
    }

    const player = this.players.get(playerId);
    if (!player || !player.isHost) {
      return { success: false, message: 'Только хост может менять настройки игры.' };
    }
  ...
  GameState.prototype.getMechanics = function getMechanics() {
    ...
    mechanics.initForGame = function initForGameWithSettings() {
      ...
      if (!config.additionalTasks) {
        this.goalGenerator.generate = (count) => Array.from({ length: count }, () => ({ id: null }));
      }

      try {
        originalInitForGame();
      } finally {
        this.goalGenerator.generate = originalGenerate;
      }

      if (!config.additionalTasks) {
        this.game.players.forEach((player) => {
          delete player.personalGoal;
          delete player.personalGoalStatus;
        });
        this.goalProgress = {};
      }
  ...
  if (originalGetFinaleMechanics) {
    GameState.prototype.getFinaleMechanics = function getFinaleMechanicsWithSettings() {
      ...
      finale.evaluate = function evaluateWithSettings(survivors) {
        ...
        const originalResources = this.game.getMechanics().resources;
        const originalMax = this.game.getMechanics().maxResources;
        this.game.getMechanics().resources = null;
        this.game.getMechanics().maxResources = null;
        try {
          return originalEvaluate(survivors);
        } finally {
          this.game.getMechanics().resources = originalResources;
          this.game.getMechanics().maxResources = originalMax;
        }
      };
      return finale;
    };
