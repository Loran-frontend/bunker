...
class RoomManager {
  constructor(io) {
    ...
    this.playerSocketMap = new Map(); // stable playerId -> active socketId
    this.sessions = new Map(); // opaque sessionId -> session record
  ...
  attachSocket(socket, roomId, playerId) {
    socket.join(roomId);
    // GameState uses player ids as the identity for private emits. A dedicated
    // Socket.IO room keeps those emits working even when the transport socket changes.
  ...
  createRoom(socket, playerName) {
    ...
    const sessionId = this.createSession(roomId, playerId, socket.id);
    const res = gameState.addPlayer(playerId, playerName);
    if (res.success) {
      this.attachSocket(socket, roomId, playerId);
      socket.emit('room:created', { roomId, state: gameState.getSanitizedState(playerId) });
  ...
  joinRoom(socket, roomId, playerName) {
    ...
    const playerId = this.generatePlayerId();
    const sessionId = this.createSession(formattedCode, playerId, socket.id);
    const res = gameState.addPlayer(playerId, playerName);
    if (res.success) {
      this.attachSocket(socket, formattedCode, playerId);
      socket.emit('game:init', gameState.getSanitizedState(playerId));
      this.emitSessionIssued(socket, sessionId, playerId);
      gameState.broadcastState();
    } else {
      this.sessions.delete(sessionId);
  ...
  transferHostOnDisconnect(gameState, playerId) {
    ...
    const nextHost = Array.from(gameState.players.values()).find((candidate) => {
      if (candidate.id === playerId || candidate.eliminated) return false;
      return !!this.playerSocketMap.get(candidate.id);
    });
      ...
      if (player) {
        if (room.status === 'VOTING') {
          ...
          for (const [voterId, targetId] of room.votes.entries()) {
            if (targetId === session.playerId) room.votes.delete(voterId);
          }
        }
        ...
        if (room.status === 'VOTING') {
          const validVoters = room.getAlivePlayers().filter((p) => {
            const mods = room.specialModifiers.get(p.id) || {};
  ...
  reconnect(socket, sessionId) {
    ...
    // A finished game is no longer an active session. Existing connected
    // clients keep their final state, but a later reconnect cannot revive it.
    ...
    // Graceful handoff: one session has one active socket. If a second socket
    // presents the valid session token, the older connection is disconnected.
    ...
    socket.emit('session:restored', {
      ...
      state: room.getSanitizedState(session.playerId),
    });
  ...
  handleDisconnect(socketId) {
    ...
    const session = Array.from(this.sessions.values()).find((item) =>
      item.roomId === roomId && item.playerId === playerId,
    );
    if (!session) return;

    // Ignore stale disconnect callbacks after a graceful socket handoff.
