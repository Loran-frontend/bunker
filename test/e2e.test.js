...
class SocketIOClient {
  ...
  handle(packet) {
    ...
    if (packet.startsWith('0')) {
      ...
      this.ws.send('40'); // Socket.IO namespace connect.
      return;
    }

    if (!packet.startsWith('4')) return;

    // Socket.IO packet after the Engine.IO message prefix.
    const socketPacket = packet.slice(1);
    if (socketPacket === '0' || socketPacket.startsWith('0{')) {
      this.emitLocal('connect');
      return;
    }
    if (socketPacket.startsWith('1')) {
  ...
  emit(event, payload) {
    const data = payload === undefined ? [event] : [event, payload];
...
function expectState(client, status) {
  assert.equal(
    client.state?.status,
    status,
    `${client.name}: expected ${status}, got ${client.state?.status}`,
  );
}
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      server.stdout.on('data', (data) => {
        if (String(data).includes('Сервер запущен')) {
          clearTimeout(timer);
          resolve();
        }
      });
    ...
    const players = Array.from(
      { length: 6 },
      (_, index) => new SocketIOClient(index === 0 ? 'Host' : `Player${index + 1}`),
    );
    ...
    players.forEach((client) => {
      ...
      client.on('action:private', (message) => {
        if (message?.title === 'Ошибка') client.errors.push(message.message);
      });
    });
    ...
    for (let index = 1; index < players.length; index += 1) {
      ...
      await waitFor(
        () => players[index].state?.players?.length === index + 1,
        3000,
        `join ${players[index].name}`,
      );
    }
    ...
    players[1].emit('game:start');
    await sleep(100);
    expectState(players[0], 'LOBBY');
    players[1].emit('game:next_phase');
    await sleep(100);
    expectState(players[0], 'LOBBY');
    ...
    // Wrong-phase special-card action must be rejected without changing phase.
    players[1].emit('card:action', {
      category: 'special1',
      targetId: players[0].state.players[0].id,
    });
    await sleep(50);
    ...
    while (players[0].state.status === 'REVEAL') {
      ...
      await waitFor(
        () => players[0].state.activePlayerId !== activeId || players[0].state.status !== 'REVEAL',
        3000,
        'next reveal turn',
      );
    }
    ...
    // Discussion: chat, alliance/trust and phase-restricted gameplay actions use server handlers.
    ...
    const remainingCandidates = players[0].state.tiedCandidates.filter((id) =>
      players[0].state.players.some((player) => player.id === id && !player.eliminated),
    );
    assert.equal(remainingCandidates.length, 2);

    // Revote target restriction: only the remaining tied candidates are valid.
    const invalidTarget = players[0].state.players.find((player) =>
      !player.eliminated && !remainingCandidates.includes(player.id),
    );
    if (invalidTarget) {
    ...
    // Continue real rounds with unanimous votes until GAME_OVER.
    let rounds = 0;
    while (players[0].state.status !== 'GAME_OVER' && rounds < 10) {
      ...
      if (players[0].state.status === 'VOTING') {
        ...
        for (const client of players) {
          const self = myPlayer(client);
          if (self && !self.eliminated && self.id !== target) client.emit('vote:cast', { targetId: target });
        }
    ...
    await waitFor(
      () => players[0].state.status === 'GAME_OVER' && players[0].finale,
      5000,
      'GAME_OVER/finale',
    );
